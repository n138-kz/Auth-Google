		...
		if ($config['internal']['databases']['activate'] && $config['internal']['databases']['primary']['activate']) {
			...
			try {
				...
				if ( $pdo_result['superuser'] || ( ( ( $pdo_result['authgoogle_userinfo'] & 4 ) === 4 ) && ( ( $pdo_result['authgoogle_sessions'] & 4 ) === 4 ) ) ) {
					$sql  = 'SELECT public.authgoogle_userinfo.email, public.authgoogle_userinfo.name, public.authgoogle_sessions.userid, public.authgoogle_sessions.address, public.authgoogle_sessions.iat, public.authgoogle_sessions.exp, public.authgoogle_sessions.token FROM public.authgoogle_sessions';
